...
export default function ComponentSet(props: IComponentSet) {
    const handleChange = (data: IItem) => {
        props.presenter.activeComponent(data.name);
    };
            ...
            <Observer>
                {
                    () => (
                        ...
                        <div key={props.presenter.currentItem.name} className={Style.container}>
                            {components.map((option) => (
                                <Option key={option.name} item={option} onClick={handleChange}
                                        isActive={option.name === props.presenter.currentItem.compType}/>
                            ))}
                        </div>
                    )
                }
